...
![The unzip location and folder of `osTicket-Installation-Files.zip` that I used here](https://safe.reku.me/osticket-prereqs/1.png)
...
![All the Windows Features you need to enable](https://safe.reku.me/osticket-prereqs/2.png)
...
![What `127.0.0.1` should look like](https://safe.reku.me/osticket-prereqs/3.png)
- Then go back to `osTicket-Installation-Files`
- Run `PHPManagerForIIS_V1.5.0.msi`

![The `PHP Manager for IIS` Setup Wizard](https://safe.reku.me/osticket-prereqs/4.jpg)
- Keep clicking until `PHP Manager for IIS` is installed.
- Go back to `osTicket-Installation-Files`
- Run `rewrite_amd64_en-US.msi`

![The `IIS URL Rewrite Module 2` Setup](https://safe.reku.me/osticket-prereqs/5.png)
- Keep clicking until `Rewrite Module` is installed.
- Go back to `osTicket-Installation-Files`
- Go to `C:`
- Create the directory `C:\PHP`

![The directory `C:\PHP`](https://safe.reku.me/osticket-prereqs/6.png)
- Go back to `osTicket-Installation-Files`
- Unzip `php-7.3.8-nts-Win32-VC15-x86.zip` to `C:\PHP`

![Unzipping `php-7.3.8-nts-Win32-VC15-x86.zip` to `C:\PHP`](https://safe.reku.me/osticket-prereqs/7.png)
- Go back to `osTicket-Installation-Files`
- Run `VC_redist.x86.exe`

![`VC_redist.x86.exe`](https://safe.reku.me/osticket-prereqs/8.png)
- Keep clicking until `VC redist` is installed.
- Go back to `osTicket-Installation-Files`
- Run `mysql-5.5.62-win32.msi`

![`MySQL` Setup](https://safe.reku.me/osticket-prereqs/9.png)
- Click next until you get to `Setup Type`
- Click`Typical`

![`Setup Type`](https://safe.reku.me/osticket-prereqs/10.png)
- Click next until you get to `Configuration Wizard
- Click `Standard Configuration`

![Configuration type](https://safe.reku.me/osticket-prereqs/11.png)
...
![`Security options`](https://safe.reku.me/osticket-prereqs/12.png)
...
![Register PHP](https://safe.reku.me/osticket-prereqs/13.png)
- Click `OK`
- On the right side, click `Restart`

![`Restart`](https://safe.reku.me/osticket-prereqs/14.png)
- Go back to File Explorer
- Go back to `osTicket-Installation-Files`
- Unzip `osTicket-v1.15.8.zip` right there

![Unzipping `osTicket-v1.15.8`](https://safe.reku.me/osticket-prereqs/15.png)
...
![`osTicket` in `c:\inetpub\wwwroot`](https://safe.reku.me/osticket-prereqs/16.png)
- Go back to `Internet Information Services (IIS) Manager`
- On the left side, go to `Sites` -> `Default Web Site` -> `osTicket`

![`Sites` -> `Default Web Site` -> `osTicket`](https://safe.reku.me/osticket-prereqs/17.png)
- Click `PHP Manager`
...
![Enabled PHP extensions](https://safe.reku.me/osticket-prereqs/18.png)
- On the right side, click `Browse *:80 (http)`

![`Browse *:80 (http)`](https://safe.reku.me/osticket-prereqs/19.png)
- You will see a checklist, and it should look like this:

![osTicket checklist](https://safe.reku.me/osticket-prereqs/20.png)
- Go back to File Explorer
- Go to `C:\inetpub\wwwroot\osTicket\include`
- Rename `ost-sampleconfig.php` -> `ost-config.php`

![`ost-config.php` in `C:\inetpub\wwwroot\osTicket\include`](https://safe.reku.me/osticket-prereqs/21.png)
...
![Removing inheritance](https://safe.reku.me/osticket-prereqs/22.png)
- Click on `Add`
- Click `Select a principal`
- Type in `Everyone` as the object name

![Adding the `Everyone` object](https://safe.reku.me/osticket-prereqs/23.png)
- Click `OK`
- Check the `Full control` box

![Checking `Full control`](https://safe.reku.me/osticket-prereqs/24.png)
...
![`System Settings`](https://safe.reku.me/osticket-prereqs/25.png)
![`Admin User`](https://safe.reku.me/osticket-prereqs/26.png)
- Go back to File Explorer
- Go back to `osTicket-Installation-Files`
- Run `HeidiSQL_12.3.0.6589_Setup.exe`

![`HeidiSQL` Setup](https://safe.reku.me/osticket-prereqs/27.png)
- Keep clicking until `HeidiSQL` is installed
- Open `HeidiSQL`
- Click on `New` in the bottom right

![Session Manager](https://safe.reku.me/osticket-prereqs/28.png)
- Click `Session in root folder`
	- User: `root`
	- Password the `MySQL` password you saved earlier (here it's `root`)

![Session manager: `root`](https://safe.reku.me/osticket-prereqs/29.png)
- Click `Open`
- Right-click the whitespace on the left, and click `Create new` -> `Database`

![`Create new` -> `Database`](https://safe.reku.me/osticket-prereqs/30.png)
- Name: `osTicket`

![osTicket DB](https://safe.reku.me/osticket-prereqs/31.png)
- Click `OK`
- Go back to your browser
- Fill out your database settings appropriately

![`Database Settings`](https://safe.reku.me/osticket-prereqs/32.png)
- Click `Install Now`
- Congratulations!

![Congratulations!](https://safe.reku.me/osticket-prereqs/33.png)
- Check `http://localhost/osTicket/scp/login.php` to make sure it looks like this:

![`http://localhost/osTicket/scp/login.php`](https://safe.reku.me/osticket-prereqs/34.png)
- Check `http://localhost/osTicket` to make sure it looks like this:

![`http://localhost/osTicket`](https://safe.reku.me/osticket-prereqs/35.png)
- Go back to File Explorer
- Go to `C:\inetpub\wwwroot\osTicket`
- Delete `C:\inetpub\wwwroot\osTicket\setup`

![`C:\inetpub\wwwroot\osTicket\setup`](https://safe.reku.me/osticket-prereqs/36.png)
- Right-click `C:\inetpub\wwwroot\osTicket\include\ost-config.php`
- Click on `Properties`
- Check `Read-only`

![`Read-only`](https://safe.reku.me/osticket-prereqs/37.png)
- Click `OK`
